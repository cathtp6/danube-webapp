...
function setLanguage(language) {
  ...
  content.querySelectorAll('[data-zh]').forEach(node => {
    node.hidden = language !== 'zh';
    node.classList.toggle('hidden', language !== 'zh');
  });
  content.querySelectorAll('[data-en]').forEach(node => {
    node.hidden = language !== 'en';
    node.classList.toggle('hidden', language !== 'en');
  });
...
function makeAssetUrlsAbsolute(root) {
  const base = new URL(SOURCE, document.baseURI);
  root.querySelectorAll('[src]').forEach(element => {
    const source = element.getAttribute('src');
    if (source && !/^(data:|https?:\/\/|\/)/i.test(source)) element.src = new URL(source, base).href;
  });
}
...
function restoreOriginalDesign(sourceDocument) {
  ...
  sourceDocument.head.querySelectorAll('style').forEach(sourceStyle => {
    ...
    style.textContent = sourceStyle.textContent.replace(/url\(\s*(['"]?)(?!data:|https?:\/\/|\/)([^'"\)]+)\1\s*\)/gi, (match, quote, path) => `url("${new URL(path.trim(), base).href}")`);
